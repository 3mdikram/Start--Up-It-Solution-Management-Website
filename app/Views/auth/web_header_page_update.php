...
<div class="row">
    <div class="col-md-6">
        <div class="card card-whpudate">
            <div class="card-header">
                <h3 class="card-title text-white">Website Header Page Update <span class="text-sm ml-3 successColor">
                        <?php if ($session->has('success')) {
                            echo $session->success;
                        }; ?>
                    </span></h3>
            </div>
            <form action="<?php echo base_url('admin/update_web_header_page'); ?>" id="form-valid" method="POST" class="requires-validation" novalidate>
                ...
                <div class="card-footer">
                    <button type="submit" id="data-update-btn" class="btn btn-whpudate text-white w-100">Update</button>
                </div>
            </form>
        </div>
    </div>
    <div class="col-md-6">
        <div class="card card-danger">
            <div class="card-header">
                <h3 class="card-title">W-Header Page Preview</h3>
            </div>
            <div class="card-body">
                <?php if(count($webnav)>0){
                    foreach($webnav as $sbar){ ?>
                    ...
                    <div class="input-group-prepend">
                        <span class="input-group-text" onclick="return confirm('Are you sure you want to delete this?')"> <a href="<?= base_url('admin/update_web_header_page/').$sbar['id'];?>"> <i class="bi bi-x-circle-fill text-danger cursor-pointer"></i></a></span>
                    </div>
                </div>
                <?php    }
                }else{
